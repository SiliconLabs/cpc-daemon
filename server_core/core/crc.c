...
static inline uint16_t sli_cpc_compute_crc16(uint16_t crc, const uint8_t data)
{
  return (uint16_t) (crc << 8) ^ crc16_xmodem_lut[((crc >> 8) ^ data) & 0xff];
}

/***************************************************************************//**
 * Computes CRC-16 XMODEM on given buffer. Software implementation.
 ******************************************************************************/
inline uint16_t sli_cpc_get_crc_sw(const uint8_t* buffer, uint16_t buffer_length)
{
  uint16_t crc = 0;

  while (buffer_length--) {
    crc = sli_cpc_compute_crc16(crc, *buffer++);
  }
